...
Selected-base snapshot `3d8a6128e70d2f641697d5ebbfd107f02fa1f671`; 135 entries after revision 2. See [inventory method and limits](100-119-rule-inventory.md#coverage-and-interpretation).

Each source link points to the existing rule check surface; its same-name test and documentation are the read-only companions. **D:** deprecated rule, deferred to preserve current-rule review capacity; this is a delivery decision, not a claim of TypeScript compatibility. **S:** existing TypeScript tests/handling and no additional gap demonstrated. **N:** no gap demonstrated in the bounded survey; re-evaluate if implementation yields evidence. **T:** named type-information or policy boundary, excluded from this syntax-only wave. **U:** excluded from this project because an upstream contributor already owns the syntax work, as directed by the [late human review](https://github.com/1000lines/eslint/pull/3#issuecomment-5649230270). A reproducible fork gap does not authorize a duplicate lane; only explicit later human direction can recommission it. Frozen, nondeprecated rules are still eligible for fork bug fixes.
...
| [no-redeclare](../../../lib/rules/no-redeclare.js) | U — snitin315 owns open [upstream #19563](https://github.com/eslint/eslint/pull/19563), verified 2026-09-12. Original TSR-02 is withdrawn: `interface A {x:number} interface A {y:number}` produces `redeclared` at 1:34 in the fork, but duplicating this work is excluded. |
...
| [no-unused-vars](../../../lib/rules/no-unused-vars.js) | U — Tanujkanti4441 owns open [upstream #19812](https://github.com/eslint/eslint/pull/19812), verified 2026-09-12. Original TSR-03 is withdrawn: `export type F = (parameter: number) => void;` produces `unusedVar` at 1:18 with a removal suggestion, but duplicating this work is excluded. |
