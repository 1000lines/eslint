...
export const markers = [
	"<!-- symphony-pr-progress:start -->",
	"<!-- symphony-pr-progress:end -->",
];
...
const markdown = value =>
	String(value)
		.replace(/[\r\n]/g, " ")
		.replace(/[\\`*_{}\[\]<>()#!|]/g, "\\$&");
const linksFor = (snapshot, id) => [
	...new Set((snapshot.prs?.[id] ?? []).map(prUrl).filter(Boolean)),
];
...
export function renderMermaid(snapshot) {
	...
	if (
		!/^(LR|RL|TB|TD|BT)$/.test(graph.direction) ||
		ids.size !== graph.nodes.length ||
		graph.nodes.some(
			node => !/^[A-Z][A-Z0-9_]*$/.test(node.id) || !node.label?.trim(),
		) ||
		...
		new Set(graph.edges.map(edge => `${edge.from}:${edge.to}`)).size !==
			graph.edges.length
	) {
		throw new Error(
			"Invalid progress topology or current node; use the shared parsed plan",
		);
	}
	...
	for (const node of graph.nodes) {
		...
		const text = [
			...
			state,
			prs.length
				? prs.map(url => `PR #${url.split("/").at(-1)}`).join(", ")
				: "no PR yet",
		];
		if (node.id === currentNode) text.push("Current PR");
		lines.push(
			`  ${node.id}["${label(text.join(" — "))}"]:::${stateClass(state)}`,
		);
	}
	...
	if (currentNode)
		lines.push(`  style ${currentNode} stroke:#8250df,stroke-width:4px`);
	for (const node of graph.nodes) {
		const [url] = linksFor(snapshot, node.id);
		if (url)
			lines.push(`  click ${node.id} href "${url}" "Open PR" _blank`);
	}
	return lines.join("\n") + "\n";
}
...
export function renderProgress(snapshot) {
	if (
		...
		throw new Error("A UTC snapshotTime is required");
	...
	if (graph) lines.push("```mermaid", graph.trimEnd(), "```", "");
		...
		lines.push(
			"Diagram omitted: fewer than three nodes or two dependencies.",
			"",
		);
	for (const node of snapshot.graph.nodes) {
		...
		if (links.length)
			lines.push(
				`${node.id}: ${links.map(url => `[PR #${url.split("/").at(-1)}](${url})`).join(", ")}.`,
			);
	}
	for (const note of snapshot.warnings ?? [])
...
export function refreshBody(body, snapshot) {
	...
	if (
		...
		throw new Error(
			"PR body must contain one ordered pair of progress markers",
		);
	}
	return (
		body.slice(0, body.indexOf(start)) +
		renderProgress(snapshot).trimEnd() +
		body.slice(body.indexOf(end) + end.length)
	);
}
...
if (
	...
	try {
		...
		if (!input || extra.length)
			throw new Error(
				"Usage: render-pr-progress.mjs SNAPSHOT.json [BODY.md]",
			);
		const snapshot = JSON.parse(readFileSync(input, "utf8"));
		if (body)
			writeFileSync(
				body,
				refreshBody(readFileSync(body, "utf8"), snapshot),
			);
