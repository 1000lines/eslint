...
const readQuery = options => {
	...
	if (!query.trim()) {
		fail(
			"GraphQL query is empty. Pass --query-file or pipe a query on stdin.",
		);
	}
	return query;
};
...
const readVariables = options => {
	if (options.variablesFile) {
		return parseJson(
			readFileSync(options.variablesFile, "utf8"),
			"variables file",
		);
	}
...
const requestLinear = async (body, options) => {
	...
	try {
		...
		fail(
			`Linear API returned non-JSON response with HTTP ${response.status}.`,
		);
	}

	if (!response.ok) {
		fail(
			`Linear API HTTP ${response.status}: ${JSON.stringify(payload, null, 2)}`,
		);
	}
	if (payload.errors?.length) {
		fail(
			`Linear GraphQL errors: ${JSON.stringify(payload.errors, null, 2)}`,
		);
	}
